...
LOCAL MODIFICATION (gaema): this module is the rebase-time port of the
``_qwen4_exp_overrides`` declaration that lived inline in ``overrides.py``
before upstream split the registry into this package. The logic is unchanged;
only the accessors moved to the ``model_override_base`` view helpers.
"""
